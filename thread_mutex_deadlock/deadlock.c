...
void *thread1(void *arg) {
    printf("t1: begin\n");
    something(10000); // working without shared resources.
    printf("t1: try to acquire L1...\n");
    pthread_mutex_lock(&L1);
    printf("t1: L1 acquired\n");
    something(4000); // working with L1
    printf("t1: try to acquire L2...\n");
    pthread_mutex_lock(&L2);
    printf("t1: L2 acquired\n");
    something(4000); // working with L2, L1
...
void *thread2(void *arg) {
    ...
    something(5000); // working with L2
    printf("                           t2: try to acquire L1...\n");
    pthread_mutex_lock(&L1);
    printf("                           t2: L1 acquired\n");
    something(8000); // working with L1, L2
    pthread_mutex_unlock(&L1);
    pthread_mutex_unlock(&L2);
