...
#define BUF_SIZE 64
#define RECV_PATH "./fifo2"
#define SEND_PATH "./fifo1"
...
void *recv(void *arg)
{
    ...
    fd = open(RECV_PATH, O_RDONLY);
    while (1) {
	memset(rbuf, 0, BUF_SIZE);
	rc = read(fd, rbuf, BUF_SIZE);
	if (rc == 0 || (rbuf[0] != '\n' && rbuf[1] == '\0')) {
...
{
    int fd, rc;
    fd = open(SEND_PATH, O_WRONLY);
    while (1) {
	memset(wbuf, 0, BUF_SIZE);
	fgets(wbuf, BUF_SIZE, stdin);
	rc = write(fd, wbuf, strlen(wbuf)+1);
